...
if __name__ == '__main__':
    ...
    # Updated regex to match files like scores.jsonl inside subdirectories
    file_pattern = re.compile(r'scores\.jsonl')
    ...
    for root, _, files in os.walk(args.input_dir):
        for filename in files:
            if file_pattern.match(filename):
                ...
                # Create the new output filename, e.g., scores.jsonl -> scores_all.jsonl
                output_filename = filename.replace('scores.jsonl', 'scores_all.jsonl')
    ...
    if found_files == 0:
        print("No matching files (e.g., scores.jsonl) were found.")
    else:
