...
class DMC(embodied.Env):
  ...
  def obs_space(self):
    ...
    if self._only_pos:
      ...
      spaces = {k: v for k, v in spaces.items() if k in basic or 'pos' in k}
  ...
  def step(self, action):
    ...
    if self._only_pos:
      # Keep only position keys and basic fields
      obs = {k: v for k, v in obs.items() if k in basic or 'pos' in k}
    key = 'image' if self._image else 'log/image'
